...
import { useCheckIfUserExistsMutation } from "@/lib/react-query/queries-and-mutations";
...
const SignIn = () => {
  ...
  const { mutateAsync: checkIfUserExists } = useCheckIfUserExistsMutation();
        ...
        >
          ...
          <div className="w-full flex flex-col gap-y-3">
            ...
            {isLoggingIn ? (
              ...
              <Button
                type="submit"
                disabled={isSubmitting || !form.getValues("email") || isLoading}
                className="w-full font-semibold rounded-xl"
              >
                {isSubmitting ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
